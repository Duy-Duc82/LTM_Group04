...
void auth_dispatch(ClientSession *sess, uint16_t cmd, const char *payload, uint32_t payload_len) {
    (void)payload_len;
    switch (cmd) {
        case 0x0101: { // REQ_REGISTER
            char *username = util_json_get_string(payload, "username");
            char *password = util_json_get_string(payload, "password");
            if (!username || !password) {
                protocol_send_error(sess, 0x0102, "INVALID_PAYLOAD");
            } else {
                AuthResult r = auth_signup(username, password);
                if (r == AUTH_OK) protocol_send_simple_ok(sess, 0x0102); // RES_REGISTER
                else if (r == AUTH_ERR_EXIST) protocol_send_error(sess, 0x0102, "USERNAME_EXISTS");
                else protocol_send_error(sess, 0x0102, "REGISTER_FAILED");
            }
            free(username); free(password);
        } break;

        case 0x0103: { // REQ_LOGIN
            char *username = util_json_get_string(payload, "username");
            char *password = util_json_get_string(payload, "password");
            if (!username || !password) {
                protocol_send_error(sess, 0x0104, "INVALID_PAYLOAD");
            } else {
                UserSession us;
                ...
                if (r == AUTH_OK) {
                    ...
                    protocol_send_response(sess, 0x0104, buf, (uint32_t)n);
                } else {
                    protocol_send_error(sess, 0x0104, "LOGIN_FAILED");
                }
            }
            free(username); free(password);
        } break;
